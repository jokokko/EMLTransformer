...
let fileVersion =
    (Environment.environVarOrDefault "APPVEYOR_BUILD_VERSION"
         (version + "." + "0"))
...
Target.create "Clean"
    (fun _ ->
    !!"src/**/bin" ++ "src/**/obj" ++ buildArtifactPath |> Shell.cleanDirs)
Target.create "Restore"
    (fun _ -> !!"src/**/*.csproj" |> Seq.iter (DotNet.restore id))
Target.create "Build" (fun _ ->
    let setParams (defaults : DotNet.BuildOptions) =
        { defaults with NoRestore = true
...
Target.create "Test" (fun _ ->
    ...
    !!"src/**/*.Tests.csproj" |> Seq.iter test)
Target.create "Package" (fun _ ->
    let props = versionArgs @ [ "--include-symbols" ]

    let setParams (c : DotNet.PackOptions) =
        { c with Configuration = configuration
